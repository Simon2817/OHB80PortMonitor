...
void InitialCommandIssuer::onRoundComplete(QList<ModbusCommand> failedCommands)
{
    ...
    LoggerManager::instance().log(AppLogger::getModbusMasterLogPath(m_masterId).toStdString(), Level::INFO, logMsg.toStdString());
...
{
    LoggerManager::instance().log(AppLogger::getModbusMasterLogPath(m_masterId).toStdString(), Level::INFO, QString("[InitialCommandIssuer]：设备ID=%1 %2").arg(m_masterId).arg(message).toStdString());
}
