...
bool QUIWidget::eventFilter(QObject *obj, QEvent *evt)
{
    ...
    // 只处理鼠标事件类型，避免对非鼠标事件做 static_cast<QMouseEvent*>（未定义行为）
    QEvent::Type evtType = evt->type();
    if (movable && (evtType == QEvent::MouseButtonPress ||
                    evtType == QEvent::MouseButtonRelease ||
                    evtType == QEvent::MouseMove ||
                    evtType == QEvent::MouseButtonDblClick)) {
        QMouseEvent *event = static_cast<QMouseEvent *>(evt);
        
        if (evtType == QEvent::MouseButtonPress) {
            if (event->button() == Qt::LeftButton) {
                mousePressed = true;
                mousePoint = event->globalPos() - this->pos();
                return true;
            }
        } else if (evtType == QEvent::MouseButtonRelease) {
            mousePressed = false;
            return true;
        ...
        } else if (evtType == QEvent::MouseButtonDblClick) {
            //以下写法可以将双击识别限定在标题栏
...
void QUIWidget::setTitleBarVisible(bool visible)
{
    if (!this->widgetTitle) {
        return;
    }
    ...
    if (visible) {
        int height = this->widgetTitle->property("_qui_title_height").toInt();
        if (height <= 0) {
            height = 30;
        }

        this->widgetTitle->setVisible(true);
        this->widgetTitle->setFixedHeight(height);
    } else {
        this->widgetTitle->setProperty("_qui_title_height", this->widgetTitle->height());
        this->widgetTitle->setFixedHeight(0);
        this->widgetTitle->setVisible(false);
    }
}

void QUIWidget::setMovable(bool movable)
{
    this->movable = movable;
}

bool QUIWidget::getMovable() const
{
    return this->movable;
}
...
void QUIWidget::on_btnMenu_Max_clicked()
{
    if (max) {
        ...
        // 使用QtHelper获取窗口当前所在屏幕的可用区域
        this->setGeometry(QtHelper::getCurrentScreenAvailableGeometry(this));
