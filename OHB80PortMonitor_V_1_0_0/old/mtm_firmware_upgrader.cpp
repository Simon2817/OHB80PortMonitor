#include "mtm_firmware_upgrader.h"
#include "modbus_tcp_master.h"
#include "mtm_foup_state_tracker.h"
#include "mtm_crc.h"
...
#include <QFileInfo>
#include "mtm_command_sender.h"
#include "mtm_command_receiver.h"
#include "mtm_timer_sender.h"
#include "qthelper.h"

// ======== 固定指令帧定义 ========
const QByteArray MtmFirmwareUpgrader::PREPARE_CMD       = QByteArray::fromHex("010600AA0000A9EA");
const QByteArray MtmFirmwareUpgrader::PREPARE_RESP      = QByteArray::fromHex("010600AA0000A9EA");
const QByteArray MtmFirmwareUpgrader::TRANSFER_OK_RESP  = QByteArray::fromHex("010600BB0000F9EF");
const QByteArray MtmFirmwareUpgrader::TRANSFER_ERR_SIZE = QByteArray::fromHex("010600BB0001382F");
const QByteArray MtmFirmwareUpgrader::TRANSFER_ERR_CRC  = QByteArray::fromHex("010600BB0002782E");
const QByteArray MtmFirmwareUpgrader::TRANSFER_ERR_FRAME= QByteArray::fromHex("010600BB0003B9EE");

MtmFirmwareUpgrader::MtmFirmwareUpgrader(ModbusTcpMaster *master, QObject *parent)
    : QObject(parent)
    , m_master(master)
    ...
    , m_firmwareVersion(master->m_firmwareVersion)
{
    ...
    connect(&m_timeoutTimer, &QTimer::timeout,
            this, &MtmFirmwareUpgrader::onTimeout);
    connect(&m_sendTimer, &QTimer::timeout,
            this, &MtmFirmwareUpgrader::onSendTimer);
}

MtmFirmwareUpgrader::~MtmFirmwareUpgrader()
{
    stop();
}
...
// setBinFileReader
// ======================================================
void MtmFirmwareUpgrader::setBinFileReader(BinFileReader *reader)
{
...
// isRunning / 参数设置器
// ======================================================
bool MtmFirmwareUpgrader::isRunning() const { return m_running; }

void MtmFirmwareUpgrader::setPrepareTimeout(int ms)  { m_commandTimeout  = ms; }
void MtmFirmwareUpgrader::setWaitingTime(int ms)     { m_waitingTime     = ms; }
void MtmFirmwareUpgrader::setSendInterval(int ms)    { m_sendInterval    = ms; }
void MtmFirmwareUpgrader::setTransferTimeout(int ms) { m_transferTimeout = ms; }

// ======================================================
// emitState — 统一状态更新 + 信号 + 日志
// ======================================================
void MtmFirmwareUpgrader::emitState(UpgradeState state, const QString &msg, const QByteArray &frame)
{
    m_state = state;
    emit stateChanged(state, msg, frame);
    LoggerManager::instance().log(m_logFile, Level::INFO, "{}{}", m_logPrefix, msg.toStdString());
}

// ======================================================
// start()  ——  固件升级入口
// ======================================================
void MtmFirmwareUpgrader::start(const QString &binFilePath)
{
    if (m_running) {
        LoggerManager::instance().log(m_logFile, Level::WARN,
            "{}[固件升级] 升级器已在运行中，忽略重复启动", m_logPrefix);
        return;
    }
    ...
    LoggerManager::instance().log(m_logFile, Level::INFO,
        "{}[固件升级] 目标版本号: {}", m_logPrefix, m_targetVersion.toStdString());
    ...
    //       如果每个 MtmFirmwareUpgrader 都自动创建自己的 BinFileReader，
    //       将会导致：
    ...
    LoggerManager::instance().log(m_logFile, Level::INFO,
        "{}[固件升级] 使用预设的 BinFileReader（已读取完成）", m_logPrefix);
    ...
    // CRC 覆盖范围：01 06 00 BB + Len(2) + 55 AA + 全部 bin 数据
    {
        ...
        m_fileCrc = MtmCrc::modbusCRC16(crcInput);
    }

    LoggerManager::instance().log(m_logFile, Level::INFO,
        "{}[固件升级] 文件大小: {} 字节，分包数: {}，CRC: {}",
        m_logPrefix, m_fileTotalSize, m_totalPackets,
        m_fileCrc.toHex(' ').toUpper().toStdString());

    // 5. 暂停 master 普通收发子模块，防止干扰固件升级期间的 socket 通信
    if (m_master->m_timerSender)     m_master->m_timerSender->stop();
    if (m_master->m_sender)          m_master->m_sender->stop();
    if (m_master->m_receiver)        m_master->m_receiver->pause();
    if (m_master->m_foupStateTracker) m_master->m_foupStateTracker->pause();
    ...
    if (m_socket && m_socket->bytesAvailable() > 0) {
        LoggerManager::instance().log(m_logFile, Level::WARN,
            "{}[固件升级] 排空 socket 残留数据 {} 字节",
            m_logPrefix, m_socket->bytesAvailable());
        m_socket->readAll();
    }
...
// stop()
// ======================================================
void MtmFirmwareUpgrader::stop()
{
    if (!m_running) return;

    LoggerManager::instance().log(m_logFile, Level::INFO,
        "{}[固件升级] 升级流程被取消", m_logPrefix);
    ...
    if (m_masterSubModulesPaused) {
        ...
        if (m_master->m_sender)           m_master->m_sender->start();
        if (m_master->m_receiver)         m_master->m_receiver->resume();
        if (m_master->m_timerSender)      m_master->m_timerSender->start();
        if (m_master->m_foupStateTracker) m_master->m_foupStateTracker->resume();
    }
}
...
// ======================================================
QString MtmFirmwareUpgrader::parseVersionFromFilename(const QString &filePath)
{
...
// ======================================================
void MtmFirmwareUpgrader::sendPrepareCommand()
{
...
// ======================================================
void MtmFirmwareUpgrader::onTimeout()
{
    switch (m_state) {
...
// ======================================================
void MtmFirmwareUpgrader::startDataTransfer()
{
    ...
    if (isOnlyFrame) {
        ...
        m_socket->write(firstFrame);
        m_socket->flush();
        emit progress((1 * 100) / qMax(1, m_totalPackets));
...
// 最后一帧：bin 数据 + CRC(2字节)
// ======================================================
void MtmFirmwareUpgrader::onSendTimer()
{
    ...
    if (!isLast) {
        ...
        m_socket->write(pkt);
        m_socket->flush();
        emit progress(((m_currentPacketIndex + 1) * 100) / qMax(1, m_totalPackets));
...
// ======================================================
void MtmFirmwareUpgrader::sendVersionCommand()
{
    QByteArray frame;
    ...
    frame.append(MtmCrc::modbusCRC16(frame.mid(2)));
...
// ======================================================
void MtmFirmwareUpgrader::onSocketReadyRead()
{
    if (!m_socket) return;
    ...
    LoggerManager::instance().log(m_logFile, Level::INFO,
        "{}[固件升级] readyRead: state={}, incoming={}, bufTotal={}",
        m_logPrefix,
...
// ======================================================
bool MtmFirmwareUpgrader::handlePrepareResponse()
{
    ...
    if (foundPos > 0) {
        LoggerManager::instance().log(m_logFile, Level::WARN,
            "{}[固件升级] 跳过缓冲区头部 {} 字节残留帧，找到准备升级响应",
            m_logPrefix, foundPos);
    }
...
// ======================================================
bool MtmFirmwareUpgrader::handleTransferResponse()
{
    LoggerManager::instance().log(m_logFile, Level::INFO,
        "{}[固件升级] SendingLastFrame readyRead: buf({}) = {}",
        m_logPrefix,
        m_receiveBuffer.size(),
        m_receiveBuffer.toHex(' ').toUpper().toStdString());
    ...
    if (foundPos > 0) {
        LoggerManager::instance().log(m_logFile, Level::WARN,
            "{}[固件升级] SendingLastFrame: 跳过缓冲区头部 {} 字节残留帧",
            m_logPrefix, foundPos);
    }
...
// ======================================================
bool MtmFirmwareUpgrader::handleVersionResponse()
{
    ...
    emit finished(matched, UpgradeState::Finished,
                  matched ? QString()
...
// ======================================================
void MtmFirmwareUpgrader::finishWithResult(bool success, UpgradeState state, const QString &errorMsg)
{
    stopAllTimers();
    disconnectSocketSignals();
    m_running = false;
    ...
    if (m_masterSubModulesPaused) {
        m_masterSubModulesPaused = false;
        if (m_master->m_sender)           m_master->sender()->start();
        if (m_master->m_receiver)         m_master->receiver()->resume();
        if (m_master->m_timerSender)      m_master->timerSender()->start();
        if (m_master->m_foupStateTracker) m_master->m_foupStateTracker->resume();
    }

    if (success) {
        LoggerManager::instance().log(m_logFile, Level::INFO,
            "{}[固件升级] 成功完成", m_logPrefix);
    } else {
        LoggerManager::instance().log(m_logFile, Level::ERROR,
            "{}[固件升级] 失败终止（状态 {}）：{}",
            m_logPrefix, static_cast<int>(state), errorMsg.toStdString());
    }

    emit finished(success, state, errorMsg);
}

// ======================================================
// 辅助方法
// ======================================================
void MtmFirmwareUpgrader::stopAllTimers()
{
    m_timeoutTimer.stop();
    m_sendTimer.stop();
}

void MtmFirmwareUpgrader::connectSocketSignals()
{
    if (m_socket) {
        connect(m_socket, &QIODevice::readyRead,
                this, &MtmFirmwareUpgrader::onSocketReadyRead,
                Qt::UniqueConnection);
        connect(m_socket, &QAbstractSocket::disconnected,
                this, &MtmFirmwareUpgrader::onSocketDisconnected,
                Qt::UniqueConnection);
    }
}

void MtmFirmwareUpgrader::disconnectSocketSignals()
{
    if (m_socket) {
...
// ======================================================
void MtmFirmwareUpgrader::onSocketDisconnected()
{
    if (!m_running) return;
    ...
    LoggerManager::instance().log(m_logFile, Level::ERROR,
        "{}{}", m_logPrefix, errMsg.toStdString());
