...
{
    // 警报级别：1位数字
    QString levelStr = QString::number(alarmLevel);

    // 来源标识：转换为5位字符串
    // 如果是数字，前面补0；如果是字符串，取前5位或补0
    QString sourceIdStr;
    bool isNumeric;
    int sourceIdNum = sourceIdentifier.toInt(&isNumeric);
    ...
    if (isNumeric) {
        ...
        // 字符串类型，取前5位，不足补0
        sourceIdStr = sourceIdentifier.left(5);
    ...
    // 警告类型：4位数字，前面补0
    QString typeStr = QString("%1").arg(alarmType, 4, 10, QChar('0'));
